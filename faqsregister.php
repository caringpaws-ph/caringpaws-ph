...
$vetFN = $vetLN = $vbID = $password = $confirm_password = $vetAddress = $vetContact = $doctorDOB = $vetEmail = "";
$vetFN_err = $vetLN_err = $vetID_err = $password_err = $confirm_password_err = $vetAddress_err = $vetContact_err = $vetDOB_err = $vetEmail_err = "";
 
// Processing form data when form is submitted
if($_SERVER["REQUEST_METHOD"] == "POST"){
    
    // Validate firstname
    if(empty(trim($_POST["vetFN"]))){
      $vetFN_err = "Please enter first name.";
  } else{
      // Prepare a select statement
      $sql = "SELECT ID FROM blogger WHERE bloggerFirstName = ?";
      
      if($stmt = $con->prepare($sql)){
          // Bind variables to the prepared statement as parameters
          $stmt->bind_param("s", $param_vetFN);
          
          // Set parameters
          $param_vetFN = trim($_POST["vetFN"]);
          
          // Attempt to execute the prepared statement
          if($stmt->execute()){
              // store result
              $stmt->store_result();
              $vetFN = trim($_POST["vetFN"]);
  ...
  // Validate lastname
  if(empty(trim($_POST["vetLN"]))){
    $vetLN_err = "Please enter last name.";
} else{
    // Prepare a select statement
    $sql = "SELECT ID FROM blogger WHERE bloggerLastName = ?";
    
    if($stmt = $con->prepare($sql)){
        // Bind variables to the prepared statement as parameters
        $stmt->bind_param("s", $param_vetLN);
        
        // Set parameters
        $param_vetLN = trim($_POST["vetLN"]);
        
        // Attempt to execute the prepared statement
        if($stmt->execute()){
            // store result
            $stmt->store_result();
            $vetLN = trim($_POST["vetLN"]);
    ...
    // Validate blogger ID
    if(empty(trim($_POST["vbID"]))){
        $vetID_err = "Please enter blogger ID.";
    } else{
        // Prepare a select statement
        $sql = "SELECT ID FROM blogger WHERE bloggerId = ?";
        
        if($stmt = $con->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("s", $param_vetID);
            
            // Set parameters
            $param_vetID = trim($_POST["vbID"]);
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    $username_err = "This blogger ID is already taken.";
                } else{
                    $vbID = trim($_POST["vbID"]);
                }
          ...
          // Validate IC Number
  if(empty(trim($_POST["vetAddress"]))){
    $vetAddress_err = "Please enter your Address";
  } else{
    // Prepare a select statement
    $sql = "SELECT ID FROM blogger WHERE bloggerAddress = ?";
    
    if($stmt = $con->prepare($sql)){
        // Bind variables to the prepared statement as parameters
        $stmt->bind_param("s", $param_vetAddress);

        // Set parameters
        $param_vetAddress = trim($_POST["vetAddress"]);
        
        // Attempt to execute the prepared statement
        if($stmt->execute()){
            // store result
            $stmt->store_result();
            $vetAddress = trim($_POST["vetAddress"]);
        } else{
            echo "Oops! Something went wrong. Please try again later.";
        }
        ...
        $stmt->close();
          ...
          // Validate IC Number
          if(empty(trim($_POST["vetContact"]))){
            $vetContact_err = "Please enter your Contact No.";
          } else{
            // Prepare a select statement
            $sql = "SELECT ID FROM blogger WHERE bloggerPhone = ?";
            
            if($stmt = $con->prepare($sql)){
                // Bind variables to the prepared statement as parameters
                $stmt->bind_param("s", $param_vetContact);
        
                // Set parameters
                $param_vetContact = trim($_POST["vetContact"]);
    ...
    // Validate email
    if(empty(trim($_POST["vetEmail"]))){
      $vetEmail_err = "Please enter your Email";
  } else{
      // Prepare a select statement
      $sql = "SELECT ID FROM blogger WHERE bloggerEmail = ?";
      
      if($stmt = $con->prepare($sql)){
          // Bind variables to the prepared statement as parameters
          $stmt->bind_param("s", $param_vetEmail);
          
          // Set parameters
          $param_vetEmail = trim($_POST["vetEmail"]);
          ...
          if($stmt->execute()){
              ...
              if($stmt->num_rows == 1){
                  $vetEmail_err = "This Email is already taken.";
              } else{
                  $vetEmail = trim($_POST["vetEmail"]);
              }
...
if(empty(trim($_POST["vetDOB"]))){
  $vetDOB_err = "Please enter Pet Birthdate";
} else{

  // Combine Y/M/D
  $doctorDOB = trim($_POST['vetDOB']);

  // Prepare a select statement
  $sql = "SELECT ID FROM blogger WHERE bloggerDOB = ?";
  
  if($stmt = $con->prepare($sql)){
      // Bind variables to the prepared statement as parameters
      $stmt->bind_param("s", $param_vetDOB);
      
      // Set parameters
      $param_vetDOB = $doctorDOB;
    ...
    if(empty($vetFN_err) && empty($vetLN_err) && empty($vetID_err) && empty($vetAddress_err) && empty($vetContact_err) && empty($vetEmail_err) && empty($password_err) && empty($confirm_password_err) && empty($vetDOB_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO blogger (bloggerId, password, bloggerFirstName, bloggerLastName, bloggerAddress, bloggerPhone, bloggerEmail, bloggerDOB) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = $con->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ssssssss", $param_vetID, $param_password, $param_vetFN, $param_vetLN, $param_vetAddress, $param_vetContact, $param_vetEmail, $param_vetDOB);
            ...
            $param_vetFN = $vetFN;
            $param_vetLN = $vetLN;
            $param_vetAddress = $vetAddress;
            $param_vetContact = $vetContact;
            $param_vetEmail = $vetEmail;
            $param_vetDOB = $doctorDOB;
            
            // Attempt to execute the prepared statement
            if($stmt->execute()){
                // Redirect to login page
                $_SESSION["success8"] = "<div style='" . "margin-bottom: 100px;" . "' class=\"alert alert-success\">Your account has been registered! Sign in now!</div>";
                header("location: faqsadmin.php");
                exit();
            } else{
                $_SESSION["failed8"] = "<div style='" . "margin-bottom: 100px;" . "' class=\"alert alert-success\">Something went wrong. Please try again later.</div>";
                header("location: faqsadmin.php");
                exit();
            }
...
<head>
    ...
    <title>CaringPaws Vet Blogger Admin Registration</title>
    <!--  jQuery -->
...
<div id="login-page">
  <div class="login">
    <h2 class="login-title">CaringPaws<br>Vet Blogger Admin Registration</h2>
    <p class="notice">Please register to access the system</p>
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
    
              <div class="input-text  <?php echo (!empty($vetFN_err)) ? 'has-error' : ''; ?>">
                <!--<label for="username">First Name</label>-->
                <input name="vetFN" type="text" class="form-control" placeholder="First name" id="fname" value="<?php echo $vetFN; ?>">
                <span class="help-block"><?php echo $vetFN_err; ?></span>
              </div>
              
              <div class="input-text  <?php echo (!empty($vetLN_err)) ? 'has-error' : ''; ?>">
                <!--<label for="username">Last Name</label>-->
                <input name="vetLN" type="text" class="form-control" placeholder="Last Name" id="lname" value="<?php echo $vetLN; ?>">
                <span class="help-block"><?php echo $vetLN_err; ?></span>
              </div>
              
              <div class="input-text <?php echo (!empty($vetID_err)) ? 'has-error' : ''; ?>">
                <!--<label for="username">IC Number</label>-->
                <input name="vbID" type="text" class="form-control" placeholder="VB. ID" id="ID" value="<?php echo $vbID; ?>">
                <span class="help-block"><?php echo $vetID_err; ?></span>
              </div>

              <div class="input-text  <?php echo (!empty($vetAddress_err)) ? 'has-error' : ''; ?>">
                <!--<label for="username">Email</label>-->
                <input name="vetAddress" type="text" class="form-control" placeholder="Address" id="address" value="<?php echo $vetAddress; ?>">
                <span class="help-block"><?php echo $vetAddress_err; ?></span>
              </div>

              <div class="input-text <?php echo (!empty($vetContact_err)) ? 'has-error' : ''; ?>">
                <!--<label for="username">Email</label>-->
                <input name="vetContact" type="text" class="form-control" placeholder="Contact No." id="contact" value="<?php echo $vetContact; ?>">
                <span class="help-block"><?php echo $vetContact_err; ?></span>
              </div>

              <div class="input-text  <?php echo (!empty($vetEmail_err)) ? 'has-error' : ''; ?>">
                <!--<label for="username">Email</label>-->
                <input name="vetEmail" type="text" class="form-control" placeholder="Email" id="email" value="<?php echo $vetEmail; ?>">
                <span class="help-block"><?php echo $vetEmail_err; ?></span>
              </div>
              <center><div name="selDate"></div><center>
              <div class="input-text <?php echo (!empty($vetDOB_err)) ? 'has-error' : ''; ?>">       
                    <input class="form-control" id="date" name="vetDOB" placeholder="YYYY/MM/DD" type="text"/>
                    <span class="help-block"><?php echo $vetDOB_err; ?></span>
              </div>
...
In order to resolve a complaint regarding the Site or to receive further information regarding use of the Site, please contact us at:
    ...
    <div class="created"><p>Have an account? <a href="faqsadmin.php">Sign In</a>.</p>
...
<script>
    $(document).ready(function(){
      var date_input=$('input[name="vetDOB"]'); //our date input has the name "date"
